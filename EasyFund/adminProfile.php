...
<?php
    ...
    if(isset($_POST['btn_submit'])){
        ...
        if($password!=""){
            $sql = "UPDATE admin SET  name ='$name', email ='$email', phone = '$phone', username ='$username', password = '$password' WHERE id = '$id'"; 
        }else{
            $sql = "UPDATE admin SET  name ='$name', email ='$email', phone = '$phone', username ='$username' WHERE id = '$id'"; 
        }

        mysqli_query($mysqli, $sql) or die('Query failed. ' . mysqli_error());
          
        print "<script>";
        print "alert('User profile updated successfully'); self.location='adminProfile.php';"; 
        print "</script>"; 
